...
export async function POST(request: NextRequest) {
	try {
		...
		// Generate store composition (update template files)
		const result = await shopifyClient.generateStoreComposition(storeData);

		return NextResponse.json({
			success: true,
			result,
			message: "Store composition updated successfully",
		});
	} catch (error) {
		console.error("Composition generation error:", error);
		return NextResponse.json(
			{
				error: "Failed to update store composition",
